...
2. 非阻塞IO模型
   ...
   请求数据时，若没有数据准备好，会直接返回，通知应用此时没有数据，不会让应用一直在此等待。因此，应用如果想每次都迅速的响应数据，需要一直不断地调用数据请求的方法。
...
5. 异步IO模型
   ...
   应用告知内核启动某个操作，并让内核在整个操作完成之后通知应用，这种模型与信号驱动模型的主要区别在于，信号驱动IO只是由内核通知我们何时可以开始下一个IO操作，而异步IO模型是由内核通知我们操作什么时候完成，将数据传送到用户态是由操作系统进行处理的。

   拿快递时去自提点拿，是信号驱动模型；而送到家门口则是异步IO模型。这里的自提点，指的就是线程拿数据的地方：TCP缓冲区。
   
   **异步IO的优化思路是解决了应用程序需要先后发送询问请求、接收数据请求两个阶段的模式，在异步IO的模式下，只需要向内核发送一次请求就可以完成状态询问和数据拷贝的所有操作。**
