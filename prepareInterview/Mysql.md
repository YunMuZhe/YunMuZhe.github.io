...
>  几种日志整理一下：https://www.cnblogs.com/myseries/p/10728533.html
...
## 不走索引的情况总结
> [参考链接](https://blog.csdn.net/sufu1065/article/details/125195401)

索引列的错误使用：
   1. 索引列参加计算
   2. 索引列使用了函数
   3. 类型隐式转换：a = '123',其中a为int型的
   4. 两个列作比较：age > id
   5. 使用了`select *`，不使用`select *`时有可能会走**索引覆盖**

关键字的部分用法：
   1. like的参数中，左侧进行了模糊匹配
   2. 使用了or关键字
   3. is not null：不会走索引，但is null会走
   4. not in的非主键索引会失效
   5. not exists的所有索引都会失效
   6. Order by的非主键索引会失效
...
其他：
   1. 不符合最左匹配原则
   2. 进行范围扫描时，若扫描的行记录数超过了全表的10~30%，可能会放弃走索引，即：走索引没有全表扫描的效率高
...
AVL树：任意两个节点的两个子树的高度差不超过1的二叉树

（1）平衡的维护成本高：维护平衡过程的成本代价很高，因为每次删除一个节点或者增加一个节点的话，需要一次或者多次的左旋，右旋等去维护“平衡”状态

（2）查询效率低：查询的效率不稳定，还是会有看运气的成分在里面

（3）高度：如果节点很多的话，AVL树的高度会很高的，查询效率还是会很低
...
2. 计算全表扫描的代价；
3. 计算使用不同索引执行查询的代价；
4. 对比各种执行方案的代价，找出其中成本最低的那个方案。

## 为什么建议创建联合索引？

建立多个索引会加大维护索引的开销，同时效率也不一定更高（有回表），而使用联合索引的话，则可以通过最左匹配原则来将其最大化的利用。
...
Buffer Poll是存在于内存中的缓冲区，当访问MySQL的数据时，MySQL会通过一些策略来将访问到的页放到buffer poll中作为缓存，后续如果再访问的话就可以直接访问缓存中的数据了，相比访问磁盘而言，可以极大地提高速度。

Buffer Poll是一段内存区域，其中分为控制块和缓冲页。每个控制块和缓冲页都是一一对应的。

innodb使用了很多链表如free链表和flush链表来管理这块区域：

free链表：每个节点都代表一个空闲的缓冲页，在将磁盘中的页刷新到Buffer Poll中时，会从free链表中寻找空闲的缓冲页。

flush链表：当某个缓冲页中的数据被修改时，相关控制块会被加入到flush链表中，被后续加载到磁盘中。

LUR链表：使用LUR策略管理当前所有已经加载进来的缓冲页。为了解决下面两种问题，这个链表被分为了young区和old区：

- 预读：innodb认为执行当前请求时，可能会在后面读取某些页面，于是就预先把这些页面加载进buffer pool中。（加载进来的页可能不会被用到）
- 全表扫描：在全表扫描时因为要访问所有数据，因此会把访问到的叶子结点的所有页都加载进来（数据量太大，把本来访问频繁的页替换出去）

首次加载进来的页放到old的头部，如果只在某个时间间隔访问，则不会被移动到young区，因为young区在前面，所以可以保证热点数据不被覆盖掉，后续有限淘汰old区的一些页。

为了快速定位某个页是否被加入了缓冲区，使用表空间号+页号作为key，缓冲页控制块的地址作为value的形式来建立hash表。
...
## CAP理论
[CAP理论](https://wenku.baidu.com/view/d86d52325c0e7cd184254b35eefdc8d376ee14f0.html)中的C指的是多节点中数据的一致性，如强一致性、弱一致性、最终一致性。

CAP的解释：

C(Consistency一致性)：数据在多个副本之间保持一致。
- 强一致性：第一个节点操作完后，去读取第二个节点的数据时，必定一致。
- 弱一致性：**尽可能**在某个时间内达到数据的一致。
- 最终一致性：**保证**在某个时间级别内达到数据的一致。

A(Availability可用性): 系统提供的服务必须一直处于可用状态。对于用户的每个操作请求总是能在**有限的时间**内**返回结果**。

P(Partition tolerance分区容错性):分布式系统在遇到任何网络分区故障时，仍需要保证对外提供满足一致性和可用性的服务，除非整个网络环境都发生了故障。
...
3. **Repeatable Read 可重复读**
   MySQL的默认级别，事务执行期间(未提交)不能读取其他事务还未提交的数据，也不能其他事务读取更新操作后并提交的数据，所以可以避免脏读和不可重复读；
   但当其他事务插入数据并提交后，若当前事务去update该记录，则对应记录trx_id就会变为当前事务，当前事务再去查询就可以查到数据了，因此不能完全避免幻读，即：

   1. 事务T1 select了一下，结果集为空
   2. 事务T2 插入了一条对应条件的数据
   3. 事务T1 运行 update/delete 语句来改动刚刚T2插入的数据
   4. 相应记录的trx_id变为T1的事务ID
   5. 事务T1再进行select，可以查询出相应的结果，幻读出现
...
由于**二级索引的节点中没有记录全量数据**，因此判断方式不同，如下：

1. 二级索引页面的Page Header部分有一个名为**Page_Max_Trx_id**的属性，其标识着**对其增删改查的最大事务id**，这也就意味着修改该二级索引页面的最大事务id是多少，进行二级索引记录的访问时，首先会通过最大增删改查事务id是否小于trx_list或者等于当前trx_id来判断可见性的。
2. 如果通过第一步判断不出来，则需要进行**回表**操作，在聚簇索引的记录上进行判断
...
基于锁的**粒度**分类：行级锁（innodb）、表级锁（innodb、myisam）、页级锁（innodb）、记录锁、间隙锁、临键锁
...
间隙锁：是属于行锁的一种，间隙锁是在事务加锁后锁住表记录的某一个区间，当表的相邻ID之间出现空隙时，则会形成一个区间，遵循左开右闭原则。范围查询并且查询未命中记录，查询条件必须命中索引，间隙锁只会出现在重复读的事务级别中。

临键锁：也属于行锁的一种，并且他是innodb的行锁默认算法，总结来说他就是记录锁和间隙锁的组合，临键锁会把查询出来的记录锁住，同时也会把该范围查询内的所有间隙空间也锁住，再之他会把相邻的下一个区间也锁住。

隐式锁：事务在表中插入数据时，因为有一个trx_id，所以可以不显式的加锁，当别的事务来访问的时候，如果发现此数据不可见，则会显示的给此数据加一个X锁（排它锁），属性is_waiting设置为false；然后自己再创建一个锁结构，is_waiting属性设置为true。
...
一个是占用空间问题：过长的数据更占用空间

一个是查询效率问题：过长的数据进行比较时更耗时，同时字符串的比较也比整数更耗时

## 选择索引时，int类型要不要自增呢？

在满足业务场景的情况下，能自增最好自增。因为新增数据时如果只在最右侧新增数据，则可以减少索引的维护成本。

![](./img/mysql/why-primary-key-unableedit.png)

如上图，左边存放的数据页已经满了，但是这时候又来了一条主键为5的数据，需要将第一页主键为8的数据移到第二页，造成了数据项的移动。
...
> 参考文章：https://juejin.cn/post/6844903840332906509

### 介绍

```sql
# limit用法：查询从fromIndex开始的dataNum条数据
select * from user limit fromIndex dataNum;
```

一般分页都是使用的limit关键字，这种方法在数据量较小时性能还是可以的，但是当表中数据量很大时就会出现问题：

limit的执行过程：在数据库中查出 fromIndex + dataNum条数据来，然后将前面的fromIndex条数据抛弃。

这种执行过程可能就会导致大量回表操作，出现大量的磁盘IO，使SQL的执行效率大大降低。

更好的做法：子查询的分页方式或者JOIN分页方式。
...
子查询是在索引上完成的，而普通的查询是在数据文件上完成的，通常来说，索引文件要比数据文件小得多，所以操作起来也会更有效率。因为要取出所有字段内容，第一种需要跨越大量数据块并取出，而第二种基本通过直接根据索引字段定位后，才取出相应内容，效率自然大大提升。
...
根据name去nameB+树上找到叶子结点，能够取到name、id的值，不需要回表了，这个过程叫做索引覆盖
...
索引下推的意思是：把查询中与被使用索引有关的搜索条件下推到存储引擎中判断，而不是返回到server层再判断。
...
1. 只适用于二级索引，不适用于聚簇索引；
2. 仅适用于select语句，不适用于update、delete这些需要改动记录的语句。

例：

select * from table where name = 1 and age = 1;（索引为(name, age)）
...
3. 从库的IO线程会将binlog日志写入到本地的relay-log中继日志中
...
所谓并行复制，知道是从库开启多个线程，来并行读取relay log中不同库的日志，然后并行重放不同库的日志，这是库级别的并行。但是由于28法则，一般高并发量的数据操作都在一个库里面，因此对于使用并行复制来解决同步延时来说意义不大。
